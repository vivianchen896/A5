public class ExperimentKeeper{
  ...
  public Data[] generateDatasetBy(int numberOfTrials, int numberOfDataPointPerTrial){
    ...
    for (int trialNum = 0; trialNum < numberOfTrials; trialNum++) {
      Data data = new Data(numberOfDataPointPerTrial);
      for (int i = 0; i < numberOfDataPointPerTrial; i++) {
       data.setPoint(i, random(1, 10), false); 
      }
      dataset[trialNum] = data;
    }
      ...
      switch(this.state){
        ...
        case STATE_TRIAL:
          if(this.canvas.hasActiveNextButtonAt(x, y)){
            ...
            float truePercentage = data.getPointPercentage();
            float reportedPercentage = constrain(Float.parseFloat(this.answer), 0, 100) / 100; 
            float error = log(abs(reportedPercentage - truePercentage) +  0.125)/log(2);
            
            println(truePercentage, reportedPercentage, error);
